...
export function getEcoliTexts() {
  return [
    ...
    ' and',
    'Shigella ',
    'data visualized in AMRnet are sourced from Enterobase (https://enteroba',
    'se.warwick.ac.uk/). Sequence types (STs) are assigned in silico via the Achtman MLST scheme, with novel STs created as needed, and unique core-genome MLST types derived from 2,513 loci. Pathovars are predicted by hierarchical clustering, while phylogroups are determined using ClermontTyper (v. 5 July 2019) and EzClermont (v. 25 August 2018). O:H serotypes (lipopolysaccharide + flagellar antigen combinations) and fimH alleles (v. 1 May 2017) are called with EcTyper and FimTyper, respectively. Antimicrobial-resistance genotypes are detected using NCBI’s AMRFinderPlus (https://www.ncbi.nlm.nih.gov/pathogens/antimicrobial-resistance/AMRFinder/). and virulence factors are identified via BlastFrost. For full details on pathovar prediction, MLST schemas, and other genotyping workflows, see the Enterobase documentation (https://enterobase.warwick.ac.uk/docs/).',

    'WARNING:',
    'The',
    ' Escherichia coli ',
    'data used in AMRnet are not yet curated for purpose-of-sampling, and therefore ',
