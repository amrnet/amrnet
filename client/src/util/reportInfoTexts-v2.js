...
export function getSalmonellaTexts(date) {
  ...
  return [
    ...
    `Source Data`,
    `TyphiNET data were last updated on ${lastUpdated}. For code and further details please see: ${githubRepo}.`,
    `Individual genome information, including derived genotype and AMR calls, sequence data accession numbers, and source information (PubMedID for citation) can be downloaded as a spreadsheet from the TyphiNET website (${typhinetURL}).`,
    `Variable definitions`,
    `The genotypes reported here are defined in ${dysonHoltPaper}.`,
    `Travel-associated cases are attributed to the country of travel, not the country of isolation, as described in ${ingleEtAlPaper}.`,
    `Antimicrobial resistance determinants are described in ${typhiPathogenwatchPaper}.`,
    `Abbreviations`,
    ...
    `3. Ciprofloxacin NS, ciprofloxacin non-susceptible (MIC >=0.06 mg/L, due to presence of one or more genes or mutations in ${careyEtAlPaper})`,
    `4. Ciprofloxacin R, ciprofloxacin resistant (MIC >=0.5 mg/L, due to presence of multiple mutations and/or genes, see ${careyEtAlPaper})`,
    `Funding`,
    `${wellcomeTrustFunding} and ${euHorizon2020Funding}.`,
  ];
}
...
export function getKlebsiellaTexts() {
  ...
  return [
    ...
    `Klebsiella pneumoniae`,
    `data in AMRnet are sourced from PathogenWatch (${pathogenwatchURL}),`,
    `which calls AMR and genotypes (MLST) from genomes assembled from public data see (${klebPathogenwatchPaperURL}). See below for details of how the genotyping is undertaken.`,
    `WARNING:`,
    `The `,
    `Klebsiella pneumoniae`,
    ...
    `Antimicrobial resistance determinants are identified by Pathogenwatch using Kleborate v2, developed and maintained by the Holt lab (${KpgithubRepo}) and described in (${lamEtAlPaper})`,
    `Abbreviations`,
...
export function getNgonoTexts() {
  ...
  return [
    ...
    `Abbreviations`,
    `1. MDR, multi-drug resistant (Resistant to one of Azithromycin / Ceftriaxone / Cefixime [category I representatives], plus two or more of Penicillin / Ciprofloxacin NS / Spectinomycin [category II representatives])`,
    `2. XDR, extensively drug resistant (Resistant to two of Azithromycin / Ceftriaxone / Cefixime [category I representatives], plus three of Penicillin / Ciprofloxacin NS / Spectinomycin [category II representatives])`,
