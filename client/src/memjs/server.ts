...
import { makeRequestBuffer, parseMessage, merge, timestamp, Message } from './utils';
...
export class Server extends events.EventEmitter {
  ...
  constructor(
    ...
    this.username =
      username || this.options.username || process.env.MEMCACHIER_USERNAME || process.env.MEMCACHE_USERNAME;
    this.password =
      password || this.options.password || process.env.MEMCACHIER_PASSWORD || process.env.MEMCACHE_PASSWORD;
    return this;
  }
  ...
  sock(sasl: boolean, go: OnConnectCallback) {
    ...
    if (!self._socket) {
      ...
      self._socket = net.connect(
        /* TODO: allowing port to be string or undefined is used by the tests, but seems bad overall. */
        typeof this.port === 'string' ? parseInt(this.port, 10) : this.port || 11211,
        this.host,
      ...
      self._socket.setTimeout(self.options.conntimeout * 1000, function (this: net.Socket) {
        self.timeoutSet = false;
        if (!self.connected) {
          this.end();
          self._socket = undefined;
