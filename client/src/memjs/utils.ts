...
export function encodeRequestIntoBuffer(buffer: Buffer, offset: number, request: EncodableRequest) {
...
export const makeAmountInitialAndExpiration = function (amount: number, amountIfEmpty: number, expiration: number) {
...
export const parseMessage = function (dataBuf: Buffer): Message | false {
  ...
  if (
    ...
    responseHeader.totalBodyLength < responseHeader.keyLength + responseHeader.extrasLength
  ) {
    return false;
  }
