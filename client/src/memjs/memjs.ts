...
import { OnErrorCallback, OnResponseCallback, Server, ServerOptions } from './server';
...
import * as constants from '../memjs/constants';
import { ResponseStatus } from '../memjs/constants';
...
type ResponseOrErrorCallback = (error: Error | null, response: Message | null) => void;
 ...
 */
type IfBuffer<Value, Extras, WhenValueAndExtrasAreBuffers, NotBuffer> = Value extends Buffer
...
export type GivenClientOptions<Value, Extras> = Partial<BaseClientOptions> &
  IfBuffer<Value, Extras, Partial<SerializerProp<Value, Extras>>, SerializerProp<Value, Extras>>;
...
export type GetMultiResult<Keys extends string = string, Value = MaybeBuffer, Extras = MaybeBuffer> = {
...
class Client<Value = MaybeBuffer, Extras = MaybeBuffer> {
  ...
  _buildGetMultiRequest(keys: string[], seq: number): Buffer {
    ...
    for (const keyIdx in keys) {
      ...
      bytesWritten += copyIntoRequestBuffer(constants.OP_GETKQ, key, '', '', seq, request, bytesWritten);
    }

    bytesWritten += copyIntoRequestBuffer(constants.OP_NO_OP, '', '', '', seq, request, bytesWritten);
   ...
   */
  async getMulti<Keys extends string>(keys: Keys[]): Promise<GetMultiResult<Keys, Value, Extras>> {
  ...
  async getMultiWithErrors<Keys extends string>(keys: Keys[]): Promise<GetMultiWithErrorsResult<Keys, Value, Extras>> {
    const serverKeytoLookupKeys: {
   ...
   * Sets `key` to `value`.
   */
  async set(key: string, value: Value, options?: { expires?: number; cas?: CASToken }): Promise<boolean | null> {
   ...
   */
  async add(key: string, value: Value, options?: { expires?: number }): Promise<boolean | null> {
    ...
    const request = makeRequestBuffer(opcode, key, serialized.extras, serialized.value, this.seq);
    const response = await this.perform(key, request, this.seq);
    switch (response.header.status) {
      case ResponseStatus.SUCCESS:
        return true;
   ...
   */
  async replace(key: string, value: Value, options?: { expires?: number }): Promise<boolean | null> {
    // TODO: support flags, support version (CAS)
    this.incrSeq();
    const expiration = makeExpiration(options?.expires || this.options.expires);
    const extras = Buffer.concat([Buffer.from('00000000', 'hex'), expiration]);
  ...
  async increment(
    ...
    const request = makeRequestBuffer(constants.OP_INCREMENT, key, extras, '', this.seq);
    const response = await this.perform(key, request, this.seq);
    switch (response.header.status) {
      case ResponseStatus.SUCCESS:
  ...
  async decrement(
    ...
    const expires = options.expires || this.options.expires;
    const extras = makeAmountInitialAndExpiration(amount, initial, expires);
    const request = makeRequestBuffer(constants.OP_DECREMENT, key, extras, '', this.seq);
    const response = await this.perform(key, request, this.seq);
    switch (response.header.status) {
      case ResponseStatus.SUCCESS:
        const bufInt = (response.val.readUInt32BE(0) << 8) + response.val.readUInt32BE(4);
  ...
  flush(callback?: (err: Error | null, results: Record<string, boolean | Error>) => void) {
  ...
  private createAndLogError(commandName: string, responseStatus: ResponseStatus | undefined): Error {
