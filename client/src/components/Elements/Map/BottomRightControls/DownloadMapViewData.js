    ...
    if (Array.isArray(genotypesDrugsData) && genotypesDrugsData.length > 0) {
      ...
      const rows = genotypesDrugsData
        ...
        .map((item) => {
          ...
          const AzithPerCount = AzithCount < 20 ? 'insufficient' : (((AzithCount/ item.totalCount) * 100).toFixed(2)) || 0;
          ...
          const CipNSPerCount = CipNSCount < 20 ? 'insufficient' : (((CipNSCount / item.totalCount) * 100).toFixed(2)) || 0;
          ...
          const TriSulPerCount = TriSulCount < 20 ? 'insufficient' : (((TriSulCount/ item.totalCount) * 100).toFixed(2)) || 0;
        ...
        .map((item) => {
            ...
            if (Array.isArray(topXGenotypeRDWG) && topXGenotypeRDWG.length > 0) {
              topXGenotypeRDWG.forEach((genotype) => {
                    ...
                    const percentage = count < 20 
                        ? 'insufficient' 
                        : ((count / item.totalCount) * 100).toFixed(2) || 0;
            ...
            .map((item) => {
                ...
                if (Array.isArray(topXGenotype) && topXGenotype.length > 0) {
                    topXGenotype.forEach((genotype) => {
                        ...
                        const percentage = count < 20 
                            ? 'insufficient' 
                            : ((count / item.count) * 100).toFixed(2) || 0;
