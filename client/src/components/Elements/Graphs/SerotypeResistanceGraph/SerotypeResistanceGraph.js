  ...
  return (
    <CardContent className={classes.serotypeResistanceGraph}>
      <Box className={classes.controlsRow}>
        <Box sx={{ display: 'flex', flexDirection: 'column' }}>
          <Box className={classes.legendBar} sx={{ padding: 0, paddingBottom: '4px' }}>
            <Typography variant="body2" fontWeight={600}>Serotype × Resistance</Typography>
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column' }}>
          <Typography variant="caption" fontWeight={600}>Vaccine filter</Typography>
          <Select value={vaccineFilter} onChange={e => setVaccineFilter(e.target.value)} size="small" sx={{ minWidth: 150, fontSize: '13px' }}>
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column' }}>
          ...
          <Select value={sortBy} onChange={e => setSortBy(e.target.value)} size="small" sx={{ minWidth: 150, fontSize: '13px' }}>
      ...
      <Box className={classes.legendBar}>
        ...
        <Box className={classes.legendGradient} sx={{ background: 'linear-gradient(to right, #e8f5e9 0%, #fff9c4 15%, #ffcc80 35%, #ef6c00 65%, #b71c1c 100%)' }} />
        <Typography variant="caption">60%+</Typography>
        <Box sx={{ marginLeft: '12px', display: 'flex', gap: '8px' }}>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: '3px' }}>
            <Box sx={{ width: 8, height: 8, backgroundColor: '#1565c0', borderRadius: '1px' }} />
            <Typography variant="caption" sx={{ fontSize: '9px' }}>PCV13</Typography>
          </Box>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: '3px' }}>
            <Box sx={{ width: 8, height: 8, backgroundColor: '#7b1fa2', borderRadius: '1px' }} />
            <Typography variant="caption" sx={{ fontSize: '9px' }}>PCV20 extra</Typography>
          </Box>
        </Box>
      </Box>
      ...
      <Box className={classes.graphWrapper}>
          ...
          {serotypes.length === 0 ? (
            <Box className={classes.noSelection}>
              <Typography variant="body2" color="textSecondary">
                {organism !== 'strepneumo' ? 'Serotype analysis available for S. pneumoniae only' : `No serotypes with N≥${MIN_SEROTYPE_COUNT}`}
              </Typography>
            </Box>
          ) : (
            <Box>
              ...
              <Box className={classes.headerRow}>
                <Box className={classes.serotypeLabel}>
                  <Typography variant="caption" fontWeight={600}>Serotype</Typography>
                </Box>
                {drugs.map(drug => (
                  <Box key={drug} className={classes.drugLabel}>
                    <Typography variant="caption" sx={{ fontSize: '9px', fontWeight: 600, writingMode: 'vertical-rl', transform: 'rotate(180deg)', whiteSpace: 'nowrap' }}>
                      {drug}
                    </Typography>
                  </Box>
                ))}
                <Box className={classes.drugLabel}>
                  <Typography variant="caption" sx={{ fontSize: '9px', fontWeight: 700, writingMode: 'vertical-rl', transform: 'rotate(180deg)', color: '#d32f2f' }}>
                    MDR
                  </Typography>
                </Box>
              </Box>
              ...
              {serotypes.map(st => {
                ...
                if (!data) return null;
                const isPCV13 = PCV13_SEROTYPES.includes(st);
                const isPCV20Extra = PCV20_EXTRA.includes(st);

                return (
                  <Box key={st} className={classes.dataRow}>
                    <Box className={classes.serotypeLabel} sx={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: '4px' }}>
                      {isPCV13 && <Box sx={{ width: 6, height: 6, backgroundColor: '#1565c0', borderRadius: '1px', flexShrink: 0 }} />}
                      {isPCV20Extra && <Box sx={{ width: 6, height: 6, backgroundColor: '#7b1fa2', borderRadius: '1px', flexShrink: 0 }} />}
                      <Typography variant="caption" sx={{ fontSize: '11px', fontWeight: isPCV13 || isPCV20Extra ? 600 : 400 }} noWrap>
                        {st}
                      </Typography>
                    ...
                    {drugs.map(drug => {
                      ...
                      return (
                        <Tooltip
                          ...
                          title={
                            <Box>
                              <Typography variant="caption" fontWeight={600}>Serotype {st} — {drug}</Typography>
                              <br />
                              <Typography variant="caption">Resistance: {value}% ({cellData?.count || 0}/{data.count})</Typography>
                            </Box>
                          }
                          arrow
                          placement="top"
                        >
                          <Box className={classes.cell} sx={{ backgroundColor: getPrevalenceColor(value) }}>
                            {value > 0 && (
                              <Typography variant="caption" sx={{ fontSize: '8px', color: getTextColor(value), fontWeight: value > 30 ? 600 : 400 }}>
                                {Math.round(value)}
                              </Typography>
                            )}
                          </Box>
                    ...
                    <Tooltip title={`MDR (≥3 drugs): ${data.mdr}%`} arrow placement="top">
                      <Box className={classes.cell} sx={{ backgroundColor: getPrevalenceColor(data.mdr) }}>
                        <Typography variant="caption" sx={{ fontSize: '8px', color: getTextColor(data.mdr), fontWeight: 600 }}>
                          {Math.round(data.mdr)}
                        </Typography>
                      </Box>
                    </Tooltip>
        ...
        <Box className={classes.rightSide}>
          <Typography variant="body2" fontWeight={600}>Vaccine Coverage</Typography>
          <Box className={classes.tooltipWrapper}>
            <Typography variant="caption" sx={{ lineHeight: 1.6 }}>
              ...
              <br /><br />
              <strong>Clinical significance:</strong> If vaccine-type serotypes carry high resistance,
              vaccination programs could reduce AMR burden by preventing resistant infections.
              Non-vaccine serotypes with high resistance may indicate serotype replacement risk.
              <br /><br />
              <strong>MDR:</strong> Resistant to ≥3 drug classes. Serotypes with high MDR rates
              are priority targets for prevention strategies.
            </Typography>
          </Box>
        </Box>
      </Box>
