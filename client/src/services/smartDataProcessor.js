...
class SmartDataProcessor {
  ...
  async processDataSmart(data, organism, dispatch) {
    ...
    if (data.length > 50000) {
      ...
      await this.processWithYieldingData(
        data,
        organism,
        allGenotypes,
        allCountries,
  ...
  async processBatchedData(data, organism, allGenotypes, allCountries, allYears, allPMID, allPathovar, allSerotype) {
    ...
    for (let i = 0; i < totalBatches; i++) {
      ...
      this.extractDataFromBatchSimple(
        batch,
        organism,
        allGenotypes,
        allCountries,
  ...
  extractDataFromBatchSimple(batch, organism, allGenotypes, allCountries, allYears, allPMID, allPathovar, allSerotype) {
    batch.forEach(record => {
      ...
      if (['sentericaints'].includes(organism) && record.seqsero2) {
        allPathovar.add(record.seqsero2);
      }

      if (['senterica'].includes(organism) && record.seqsero2) {
        allPathovar.add(record.seqsero2);
      }
    });
  }
